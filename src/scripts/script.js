...
const totalCounter = document.getElementById('quant');
const completedCounter = document.getElementById('done');
...
function updateCounters() {
    ...
    completedCounter.textContent = `${completedTasks} de ${totalTasks}`;
}

// Função para alternar visibilidade das mensagens padrão
function toggleDefaultMessages(show) {
    const display = show ? 'block' : 'none';
    clipboardIcon.style.display = display;
    phrases.forEach(phrase => phrase.style.display = display);
}
    ...
    taskContainer.innerHTML = `
        ...
        ">
            <label style="
                ...
                white-space: nowrap;
            ">
                <input type="checkbox" class="task-checkbox" style="
                    width: 18px;
                    height: 18px;
...
function addTask() {
    ...
    if (totalTasks === 0) {
        toggleDefaultMessages(false);
    }

    const task = createTask(text);
    activityList.prepend(task);
...
activityList.style.alignItems = 'center';
